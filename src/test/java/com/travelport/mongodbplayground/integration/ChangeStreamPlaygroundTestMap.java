...
@Slf4j
public class ChangeStreamPlaygroundTestMap {
    ...
    public void insertYearbook(YearbookMap yearbook) {
        try {
    ...
    public void updateYearbook(YearbookMap yearbook) {
        try {
            var mongoCollection = mongoDAO.getCollection(collectionName);
            var yearbookDoc = Document.parse(yearbook.toJson());
    ...
    public void testAddChangeStream() throws JsonProcessingException {
        var yearbook = Document.parse(YearbookUtils.getYearbookMap().toJson());
    ...
    public void testChangeStreamUpdateField() {
        ...
        var ses = Executors.newSingleThreadScheduledExecutor();
        ses.schedule(() -> {
            var yearbook = YearbookUtils.getYearbookMap();
            try {
                log.info("YearbookMap as Json: {}", yearbook.toJson());
            } catch (JsonProcessingException e) {
                e.printStackTrace();
            }
    ...
    public void testChangeStreamUpdateBensAge() {
        ...
        ses.schedule(() -> {
            ...
            log.info("Insert complete");
            try {
                yearbook.getPeople().get("Ben").setAge(19);
                updateYearbook(yearbook);
                log.info("Update complete");
            } catch (Exception e) {
                log.info("Exceptions: {}", e);
